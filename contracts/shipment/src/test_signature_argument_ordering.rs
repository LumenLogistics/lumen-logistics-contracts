...
fn setup() -> Ctx {
    ...
    let shipment_id = env.register(OrbitHaulShipment, ());
    let client = OrbitHaulShipmentClient::new(&env, &shipment_id);
...
fn proposal_second_approver_appended_after_first() {
    ...
    let client2 = OrbitHaulShipmentClient::new(&env, &env.register(OrbitHaulShipment, ()));
