...
fn setup() -> Ctx {
    ...
    let shipment_id = env.register(LumenShipment, ());
    let client = LumenShipmentClient::new(&env, &shipment_id);
...
fn setup_multisig_2of3() -> (Env, LumenShipmentClient<'static>, Address, Address, Address) {
    let (env, admin) = test_utils::setup_env();
...
fn proposal_second_approver_appended_after_first() {
    ...
    let client2 = LumenShipmentClient::new(&env, &env.register(LumenShipment, ()));
